...
def args():
    ...
    # parser.add_argument('-proxy','--proxy', help='参考masscan速率rate', required=True)
...
def masscan2httpx2nuclei(args):
    ...
    os.system('masscan -iL ' + input_file + ' -p' + port +" --rate "+rate+' -oL masscan.txt')
...
def masscan2httpx2nuclei_main():
    ...
    if os.path.getsize("masscan.txt") == 0:
        ...
        masscanfile.close()
    if os.path.exists("masscan.txt"):
        os.system('./httpx/httpx -l masscanconvert.txt -nc -o httpxresult.txt')
